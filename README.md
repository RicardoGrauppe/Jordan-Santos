...
3. **Revisão do Jordan** (`/revisar-contrato?id=…`, admin-only): ele confere o contrato inteiro e confirma com uma assinatura (cursiva com o nome, padrão, ou desenhada na mão) — isso não manda nada ao casal, só libera o botão **"Copiar link"** de volta no `/estudio`, que marca o contrato como enviado e abre um modal com o link + a mensagem pronta pra copiar ou abrir direto na conversa do WhatsApp. O mesmo botão recupera o link depois, sem gerar contrato novo — e **continua valendo depois de o casal assinar**: aí ele vira **"Copiar link da via"**, com a mensagem trocada pra segunda via, porque o mesmo link passa a abrir a tela de baixar o PDF assinado. Só a primeira liberação escreve `enviado_em`; pegar o link de novo não regride status nem reescreve data.
